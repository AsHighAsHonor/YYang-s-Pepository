...
NSString *const PPNumberTextFieldWillBeginEditNotification = @"PPNumberTextFieldWillBeginEditNotification";
NSString *const PPTextField = @"PPTextField";
...
#define PPLog(...)
#endif
NSString *const PPNumberButtonWasClickedNotification = @"PPNumberButtonWasClickedNotification";
...
- (void)setupUI
{
    ...
    _textField.font = [UIFont fontWithName:ROBOTO_MEDIUM size:_inputFieldFont];
    _textField.textColor = DARK_GRAY_COLOR;
    if (self.decimalNum) {
...
- (void)layoutSubviews
{
    ...
    if (!self.isModify) {
        ...
        _decreaseBtn.frame = CGRectMake(0, 0, _width* 2/5, _height);
        _textField.frame = CGRectMake(CGRectGetMaxX(_decreaseBtn.frame), 10, _width-2*_decreaseBtn.frame.size.width , (_height-20)/2);
        _titleLabel.frame = CGRectMake(_textField.frame.origin.x , CGRectGetMaxY(_textField.frame), _textField.frame.size.width, _textField.frame.size.height);
        _increaseBtn.frame = CGRectMake(CGRectGetMaxX(_textField.frame), 0, _width* 2/5, _height);
    }
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    if (self.disableInput) {
        [[NSNotificationCenter defaultCenter]postNotificationName:PPNumberTextFieldWillBeginEditNotification object:nil userInfo:@{PPTextField : textField}];
        return NO;
    }else{
        return YES;
    }
...
-(void)setModify:(BOOL)modify{
    _modify = modify;
    if (_modify) {
        _decreaseBtn.layer.borderWidth = 0;
        _decreaseBtn.layer.borderColor = nil;
        _increaseBtn.layer.borderWidth = 0;
        _increaseBtn.layer.borderColor =nil;
    }
}

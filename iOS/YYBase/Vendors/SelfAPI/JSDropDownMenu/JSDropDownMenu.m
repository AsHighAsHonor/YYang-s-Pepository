...
//        _cellTextLabel.font = [UIFont systemFontOfSize:14.0f];
        ...
        _cellTextLabel.font = [UIFont fontWithName:ROBOTO_REGULAR size:14];
...
-(void)setCellText:(NSString *)text align:(NSString*)align{
    ...
    // 只取宽度
    CGSize textSize = [text textSizeWithFont:[UIFont fontWithName:ROBOTO_REGULAR size:14] constrainedToSize:CGSizeMake(MAXFLOAT, 14) lineBreakMode:NSLineBreakByWordWrapping];
...
//        CAShapeLayer *indicator = [self createIndicatorWithColor:self.indicatorColor andPosition:CGPointMake(titlePosition.x + title.bounds.size.width / 2 + 8, self.frame.size.height / 2)];
        ...
        CAShapeLayer *indicator = [self createIndicatorWithColor:self.indicatorColor andPosition:CGPointMake(title.position.x + title.bounds.size.width / 2 + 8, self.frame.size.height / 2)];
...
- (CATextLayer *)createTextLayerWithNSString:(NSString *)string withColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    layer.string = string;
    layer.fontSize = 14.0;
    layer.font = (__bridge CFTypeRef _Nullable)([UIFont fontWithName:ROBOTO_REGULAR size:14]);
//    layer.alignmentMode = kCAAlignmentCenter;
    layer.alignmentMode = kCAAlignmentLeft; // !!!: kCAAlignmentLeft
    layer.foregroundColor = color.CGColor;
    
    layer.contentsScale = [[UIScreen mainScreen] scale];
    
    CGPoint newPoint =  CGPointMake(15 + size.width/2, point.y);
...
//    NSDictionary *dic = @{NSFontAttributeName: [UIFont systemFontOfSize:fontSize]};
    ...
    NSDictionary *dic = @{NSFontAttributeName: [UIFont fontWithName:ROBOTO_REGULAR size:fontSize]};
...
- (void)backgroundTapped:(UITapGestureRecognizer *)paramSender
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(backgroundDidClicked)]) {
        [self.delegate backgroundDidClicked];
    }
...
//    titleLabel.font = [UIFont systemFontOfSize:14.0];
    titleLabel.font = [UIFont fontWithName:ROBOTO_REGULAR size:14];
...
//        textSize = [titleLabel.text textSizeWithFont:[UIFont systemFontOfSize:14.0f] constrainedToSize:CGSizeMake(MAXFLOAT, 14) lineBreakMode:NSLineBreakByWordWrapping];
        textSize = [titleLabel.text textSizeWithFont:[UIFont fontWithName:ROBOTO_REGULAR size:14] constrainedToSize:CGSizeMake(MAXFLOAT, 14) lineBreakMode:NSLineBreakByWordWrapping];
        
    }
    
    cell.backgroundColor = [UIColor whiteColor];
    cell.textLabel.font = [UIFont fontWithName:ROBOTO_REGULAR size:14];
...
// !!!:  Set menu title when menu table view did selected
- (void)confiMenuWithSelectRow:(NSInteger)row leftOrRight:(NSInteger)leftOrRight{
    ...
    CGPoint newPoint =  CGPointMake(15 + size.width/2, title.position.y);

    title.position = newPoint;

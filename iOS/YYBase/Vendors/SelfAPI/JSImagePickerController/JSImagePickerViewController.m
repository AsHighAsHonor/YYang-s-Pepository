...
- (void)setupView
{
    ...
    if (IPHONEX) {
        self.imagePickerFrame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height - imagePickerHeight - 39, [UIScreen mainScreen].bounds.size.width, imagePickerHeight);
    } else {
...
- (void)imagePickerViewSetup
{
    ...
    if (self.buttonTextFont) {
        btnFont = self.buttonTextFont;
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.isSingleSelection) {
        [self selectFromLibraryWasPressed];
    }else{
        [self updateButtons];
    }
}
...
-(void)setButtonTextFont:(UIFont *)buttonTextFont{
    ...
    for (UIButton *btn in @[ self.photoLibraryBtn, self.cameraBtn, self.cancelBtn ]) {
        [btn.titleLabel setFont:buttonTextFont];
    }
}

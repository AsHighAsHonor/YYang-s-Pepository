...
+ (NSString *)humanBool:(BOOL)mechineBool{
    NSString *res = mechineBool? @"YES":@"NO";
    NSLog(@"HumanBool:-----%@",res);
    return res;
}

+ (NSString *)interceptOrginalString:(NSString *)orginalString andMarkStr:(NSString *)mark{
    NSRange range = [orginalString rangeOfString:mark];
    NSString *sub = [orginalString substringToIndex:range.location];
    return sub;
}
...
+ (NSString *)transferToASCII:(NSString *)originStr{
    ...
    for (int i=0; i < originStr.length; i++) {
        int asciiCode = [originStr characterAtIndex:i];
        [result appendFormat:@"%i",asciiCode];
        [result appendString:@";"];
    }
    return [result copy];
}

...
extension Reactive where Base: Demo {
    ...
    var delegate: DelegateProxy<Demo, DemoDelegate> {
        return RxDelegateDemoDelegateProxy.proxy(for: base)
    }
    // 实现这个，创建的类（本文是Demo）类的对象就可以点出rx了
    var didSetName: ControlEvent<String> {
         ...
         */
        let source = delegate.methodInvoked(#selector(DemoDelegate.demoDelegateMethod(d:didSetName:))).map({ (a:[Any]) -> String in
...
class RxDelegateDemoDelegateProxy: DelegateProxy<Demo, DemoDelegate>, DelegateProxyType, DemoDelegate {
    
    
    static func registerKnownImplementations() {
        self.register {RxDelegateDemoDelegateProxy(parentObject: $0, delegateProxy: RxDelegateDemoDelegateProxy.self) }
    }
    
    static func currentDelegate(for object: Demo) -> DemoDelegate? {
        return object.delegate
    }
    
    static func setCurrentDelegate(_ delegate: DemoDelegate?, to object: Demo) {
